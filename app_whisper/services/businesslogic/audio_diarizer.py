...
class SpeakerDiarizer:
    ...
    async def create_speaker_segments(self, 
        ...
        try:
            ...
            # Step 5c: Resolve overlapping speech periods
            logger.info("Step 5c: Resolving overlapping speech periods")
            resolved_segments = self._resolve_overlapping_speech(sorted_segments)
            
            # Step 5d: Merge consecutive segments from same speaker
            logger.info("Step 5d: Merging consecutive segments from same speaker")
            merged_segments = self._merge_consecutive_segments(resolved_segments)
    ...
    def _resolve_overlapping_speech(self, sorted_segments: List[SpeakerSegment]) -> List[SpeakerSegment]:
        """
        Resolves overlapping speech by truncating the earlier segment.
        If two segments from different speakers overlap, the segment that starts first
        is truncated to end where the second segment begins. This preserves all text
        and avoids deleting segments.
        
        Args:
            sorted_segments: List of SpeakerSegments sorted by start_time
            
        Returns:
            List of SpeakerSegments with overlaps resolved.
        """
        ...
        try:
            ...
            for i in range(1, len(sorted_segments)):
                ...
                # Check for overlap with a different speaker
                if (current_segment.speaker_id != next_segment.speaker_id and 
                    current_segment.end_time > next_segment.start_time):
                    
                    overlap_duration = current_segment.end_time - next_segment.start_time
                    logger.info(f"Resolving {overlap_duration:.2f}s overlap between "
                               f"{current_segment.speaker_id} and {next_segment.speaker_id}")
                    
                    # Truncate the current (earlier) segment to avoid overlap
                    current_segment.end_time = next_segment.start_time
                
                # Add the processed (and possibly truncated) segment to the list
                # if it's still valid
                if current_segment.end_time - current_segment.start_time >= self.min_segment_duration:
                    resolved.append(current_segment)
                else:
                    logger.warning(f"Segment for {current_segment.speaker_id} removed after overlap "
                                   f"resolution because it became too short.")
            ...
            logger.info(f"Overlap resolution complete: {len(sorted_segments)} -> {len(resolved)} segments")
            
            return resolved

        except Exception as e:
            logger.error(f"Error resolving overlapping speech: {str(e)}")
            return sorted_segments # Return original on failure
