...
async def process_data(message_body: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if success:
            ...
            internal_result = InternalReasonerResult(
                id=form_data.id,
            ...
            logger.info(f"Reasoner form {form_data.id} contains PII or CID: {internal_result.contains_pii_or_cid}")
            ...
            logger.info(f"Successfully processed reasoner {form_data.id}")
            ...
            logger.info(f"Failed reasoner {form_data.id} contains PII or CID: {contains_pii_or_cid}")
            ...
            logger.error(f"Failed to process reasoner {form_data.id}")
            return output_data.model_dump()
